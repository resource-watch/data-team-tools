...
# Application for which you want to clone from and to. The source application
# does not apply to datasets (it is cloned regardless) but only to layers,
# widgets, metadata and vocabularies
source_application = 'rw'
cloned_application = 'rw'

# do you want to clone only the default editable widget or all the widgets?
clone_default_widget_only = False

# Do you want to do a dry run and just print info on what will be done?
dry_run = False
        ...
        if clone_children:
            ...
            if len(layers) > 0:
                if clone_first_layer_only:
                    l = layers[0]
                    if source_application in l.attributes['application']:
                        try:
                            layer_name = l.attributes['name']
                            print('Cloning layer: {}'.format(layer_name))
                            if not dry_run:
                                l.clone(token=token, env='production',
                                        layer_params={
                                            'name': layer_name, 
                                            'application': [cloned_application]
                                        },
            ...
            if len(vocabs) > 0:
                for v in vocabs:
                    vocab = v.attributes
                    print('Vocab application - ', vocab['application'])
                    if vocab['application']==source_application:
                        print(f"Cloning vocab {vocab['name']}")
                        vocab_payload = {
                            'application': cloned_application,
                            'name': vocab['name'],
                            'tags': vocab['tags']
            ...
            if len(metas) > 0:
                for m in metas:
                    meta = m.attributes
                    print('Metadata application - ', meta['application'])
                    if meta['application']==source_application:
                        print(f'Cloning metadata {meta["name"]}')
                        if not dry_run:
                            meta_payload = {
                                "dataset": clone_dataset.id,
                                'application': cloned_application,
